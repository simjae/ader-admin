export default function SideBar(status ,barStatus) {
    this.status = status;
    ...
    const statusId = `${status}-${randomNum}`;
    let sideBox = document.querySelector(`#${sideId} .side__box`);

    this.makeSidebar = () => {
        const docflag = document.createDocumentFragment();
        let sideContent = "";
        sideWrap.className = "side__background";
        sideContent =`<div id="${sideId}" class="side__wrap"><div class="side__box"></div></div>`;
    ...
    this.openSidebar= () => {
        ...
        let sideWrap = document.querySelector(`#${sideId}`);
    ...
    this.makeContent = () => {
        if(this.status === "basket"){
            let contentHtml = 
            `<section id="${statusId}" class="basket__wrap">
                <div class="list__box">
