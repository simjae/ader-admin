...
(function($) {
            ...
            searchButton.click(function(event) {
                ...
                ajaxSuccess = function(data, textStatus, jqXHR) {
                    ...
                    // 검색 결과가 있는 경우...
                        ...
                        //커스텀 디자인 추가라인 
                        $(".address").wrap("<div class='addr-row' />")
                        $(".old_addresses").wrap("<div class='old_addr-row' />")
                        $('.addr-row').prepend("<span>도로명</span>");
                        
                        $('.old_addr-row').prepend("<span>지번</span>");
                        $('.addr-row').css({display:'flex',gap:'10px'});
                        $('.old_addr-row').css({display:'flex',gap:'20px'});
    ...
    $.fn.postcodifyPopUp = function(options) {
        ...
        return this.each(function() {
            ...
            var placeholder = $('<div class="postcodify_placeholder"></div>').text('예) 성동구 연무장길 53, 성수동2가 315-57');
