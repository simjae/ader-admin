...
if($sel_idx != null){
    ...
    foreach($db->fetch() as $data) {
		...
		if((strlen($td_sub_idx_str) + strlen($td_sub_idx_str)) > 0){
			$sub_material_str = $td_sub_idx_str.','.$delivery_sub_idx_str;
		...
		foreach($db->fetch() as $sub_material_data){
			$sub_material_info[] = array(
				...
				'sub_material_name'			=>$sub_material_data['SUB_MATERIAL_MEMO']
			);
		}
