...
/*
 +=============================================================================
 | 
 | 에디토리얼 관리 페이지 - FTP 서버 내 에디토리얼 썸네일/컨텐츠 체크
 | -----------
 |
 | 최초 작성	: 손성환
 | 최초 작성일	: 2022.08.03
...
if (!empty($ftp_thmb_web_file) && !empty($ftp_thmb_mobile_file) && !empty($ftp_cnts_web_file) && !empty($ftp_cnts_mobile_file)) {
	...
	$json_result['data'] = array(
		'thmb_web_cnt' => count($ftp_thmb_web_file),
		'thmb_mobile_cnt' => count($ftp_thmb_mobile_file),
		'cnts_web_cnt' => count($ftp_cnts_web_file),
		'cnts_mobile_cnt' => count($ftp_cnts_mobile_file),
	);
} else {
	ftp_close($conn);
