...
if ($page_idx != null && $ftp_dir != null) {
	...
	$runway_dir = $tmp_arr[intval(count($tmp_arr) - 1)];
	
	$server_img_path = "/var/www/admin/www/images/posting/runway/".$runway_dir;
	$server_vid_path = "/var/www/admin/www/videos/posting/runway/".$runway_dir;
	...
	try {
		...
		foreach($size_type as $size_type_val) {
			...
			for ($i=0; $i<count($ftp_thmb_file); $i++) {
				...
				if (in_array($thmb_ext,$img_ext_arr) || in_array($thmb_ext,$vid_ext_arr)) {
					...
					$tmp_file_name = $server_dir."/".$thmb_type."_".$runway_dir."_THMB_".time().".".$thmb_ext;
					...
					if($res == true){
						...
						if (!empty($thumb_idx)) {
							if (in_array($cnts_ext,$img_ext_arr) || in_array($cnts_ext,$vid_ext_arr)) {
								...
								//$tmp_file_name = $server_dir.$ftp_file_name;
								$tmp_file_name = $server_dir."/".$cnts_type."_".$runway_dir."_CNTS_".time().".".$cnts_ext;
								...
								if($res == true){
									...
									if (!empty($contents_idx)) {
										...
										$json_result['code'] = 301;
										$json_result['msg'] = "런웨이 컨텐츠 등록에 실패했습니다. 업로드 하려는 컨텐츠 이미지를 확인해주세요.";
										return $json_result;
									}
								}
							}
							...
							$json_result['code'] = 301;
							$json_result['msg'] = "런웨이 썸네일 등록에 실패했습니다. 업로드 하려는 썸네일 이미지를 확인해주세요.";
							return $json_result;
						}
					}
					else{
						$json_result['code'] = 301;
						$json_result['msg'] = "런웨이 썸네일/컨텐츠 파일등록에 실패했습니다. 업로드 하려는 썸네일 이미지를 확인해주세요.";
						return $json_result;
					}
				}
			}
		...
		$json_result['msg'] = "런웨이 썸네일/컨텐츠 등록에 성공했습니다.";
	} catch(mysqli_sql_exception $exception){
		$db->rollback();
		print_r($exception);
		
		$json_result['code'] = 301;
		$json_result['msg'] = "런웨이 썸네일/컨텐츠 등록에 실패했습니다.";
	}
}
