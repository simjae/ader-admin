...
$where = "ES.STANDBY_IDX = ".$standby_idx." AND ES.COUNTRY='".$country."' ";
$where_cnt = $where;

if($option_idx != null){
    $where .= " AND ES.OPTION_IDX = ".$option_idx." ";
}

if($member_name != null){
    $where .= " AND ES.MEMBER_NAME LIKE '%".$member_name."%' ";
}

if($purchase_flg != null){
    $where .= " AND ES.PURCHASE_FLG = ".$purchase_flg." ";
}

if ($apply_start_date != null && $apply_end_date != null) {
    $where .= " AND (ES.CREATE_DATE BETWEEN '".$apply_start_date."' AND '".$apply_end_date."') ";
}

$total = $db->count("      ENTRY_STANDBY    ES	LEFT JOIN
                            ORDER_INFO	  OI 
                            ON
                            ES.ORDER_IDX = OI.IDX", $where);
$total_cnt = $db->count("  ENTRY_STANDBY    ES	LEFT JOIN
                            ORDER_INFO	  OI 
                            ON
                            ES.ORDER_IDX = OI.IDX", $entry_where_cnt);
...
if ($country != null && $standby_idx != null) {
	$select_draw_sql = "
		SELECT 
			...
			ES.CREATE_DATE,
			OI.ORDER_CODE,
			(SELECT PRODUCT_CODE FROM SHOP_PRODUCT WHERE ES.PRODUCT_IDX = PRODUCT_IDX LIMIT 1) AS PRODUCT_CODE,
			OI.ORDER_STATUS,
