...
$where = "EP.PREORDER_IDX = ".$preorder_idx." AND EP.COUNTRY='".$country."' ";
$where_cnt = $where;

if($option_idx != null){
    $where .= " AND EP.OPTION_IDX = ".$option_idx." ";
}

if($member_name != null){
    $where .= " AND EP.MEMBER_NAME LIKE '%".$member_name."%' ";
}
if ($apply_start_date != null && $apply_end_date != null) {
    $where .= " AND (EP.CREATE_DATE BETWEEN '".$apply_start_date."' AND '".$apply_end_date."') ";
}

$total = $db->count("      ENTRY_PREORDER    EP	LEFT JOIN
                            ORDER_INFO	  OI 
                            ON
                            EP.ORDER_IDX = OI.IDX", $where);
$total_cnt = $db->count("  ENTRY_PREORDER    EP	LEFT JOIN
                            ORDER_INFO	  OI 
                            ON
                            EP.ORDER_IDX = OI.IDX", $entry_where_cnt);
...
if ($country != null && $preorder_idx != null) {
	$select_preorder_sql = "
		SELECT 
			...
			EP.CREATE_DATE,
			OI.ORDER_CODE,
			(SELECT PRODUCT_CODE FROM SHOP_PRODUCT WHERE EP.PRODUCT_IDX = PRODUCT_IDX LIMIT 1) AS PRODUCT_CODE,
			OI.ORDER_STATUS,
