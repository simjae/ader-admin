...
$tables =
"   dev.MEMBER_".$country." AS MEMBER   LEFT JOIN
    (SELECT 
        *
    FROM
        dev.VOUCHER_ISSUE
...
if($search_month != null){
    $where .= " AND (DATE_FORMAT(MEMBER.MEMBER_BIRTH, '%m') = '" . $search_month . "') ";
}
if($member_id != null){
    $where .= ' AND (MEMBER.MEMBER_ID LIKE "%'.$member_id.'%") ';
}
...
$sql = "SELECT
                VI.VOUCHER_IDX,
                IFNULL(VI.VOUCHER_ISSUE_CODE,'미발급') AS VOUCHER_ISSUE_CODE,
                ...
                DATE_FORMAT(MEMBER.MEMBER_BIRTH, '%Y-%m-%d') AS MEMBER_BIRTH,
                VI.MEMBER_IDX,
                MEMBER.TEL_MOBILE, 
                IFNULL(MEMBER.MEMBER_ID,'-') AS MEMBER_ID,
