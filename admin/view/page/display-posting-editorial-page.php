...
<div class="content__card regist__wrap">
	...
	<div class="card__body">
		<div class="content__wrap">
			...
			<div class="content__row">
				<input id="ftp_dir" type="text" name="ftp_dir" placeholder="/ader_prod_img/posting/editorial" style="padding-left:5px;">
...
<div class="content__card regist__wrap">
	...
	<div class="card__body">
		<div class="content__wrap">
			...
			<div class="content__row">
				<div class="btn" onClick="openEditorialProductModal();">상품검색</div>
			</div>
		</div>
		
		<div id="editorial_product_wrap">
...
function getEditorialInfoList(){
			...
			if(d.code == 200){
				...
				if (data != null) {
					...
					data.forEach(function (row){
						...
						strDiv += '				<div class="btn" onclick="checkEditorialDisplayNum(this)" display_num="' + row.display_num + '"  action_type="up">';
						strDiv += '					<i class="xi-angle-up"></i>';
						strDiv += '					<span class="tooltip top">위로</span>';
						strDiv += '				</div>';
						strDiv += '				<div class="btn" onclick="checkEditorialDisplayNum(this)" display_num="' + row.display_num + '"  action_type="down">';
						strDiv += '					<i class="xi-angle-down"></i>';
						strDiv += '					<span class="tooltip top">아래로</span>';
						strDiv += '				</div>';
						strDiv += '				<button class="btn" display_num="' + row.display_num + '" thumb_idx="' + row.thumb_idx + '" contents_idx="' + row.contents_idx + '" onclick="deleteEditorialImage(this)">삭제</button>';
						strDiv += '			</div>';
						strDiv += '		</div>';
	...
	getEditorialProductList(page_idx);
}

function getEditorialProductList(page_idx) {
	let div_wrap = $('#editorial_product_wrap');
	
	$.ajax({
		type: "post",
		url: config.api + "display/posting/editorial/product/get",
		data: {
			'page_idx' : page_idx
		},
		dataType: "json",
		error: function() {
			alert("에디토리얼 상품 조회처리에 실패했습니다.");
		},
		success: function(d) {
			if(d.code == 200) {
				div_wrap.html('');
				...
				if (data != null) {
					...
					data.forEach(function(row) {
						...
						strDiv += '<div class="table table__wrap editorial__item" style="width:48%;float:left;border:1px solid #bfbfbf;border-radius:5px;margin-right:15px;padding:15px;">';
						...
						strDiv += '        <div></div>';
						strDiv += '        <div>';
						strDiv += '            <button class="btn" onclick="deleteEditorialProduct(' + row.product_idx + ')">삭제하기</button>';
						strDiv += '        </div>';
						strDiv += '    </div>';
...
function checkProductDisplayNum(action_type,recent_idx,recent_num) {
    item_cnt = $('.editorial__item').length;
...
function putDisplayNum(action_type,recent_idx,recent_num) {
	$.ajax({
		type: "post",
		url: config.api + "display/posting/editorial/product/put",
		data: {
		...
		dataType: "json",
		error: function() {
			alert('에디토리얼 상품 진열순서 변경처리중 오류가 발생했습니다.');
		},
		success: function(d) {
			let code = d.code;
			if (code == 200) {
...
function deleteEditorialProduct(product_idx) {
	confirm(
		'선택하신 상품을 에디토리얼 상품에서 제외하시겠습니까?',
		function(){
			$.ajax({
				type: "post",
				data: {
				...
				dataType: "json",
				url: config.api + "display/posting/editorial/product/delete",
				error: function() {
					alert("에디토리얼 상품 삭제처리중 오류가 발생했습니다.");
				},
				success: function(d) {
					if(d.code == 200) {
						alert('선택한 에디토리얼 상품이 삭제되었습니다.');

						getEditorialProductList(page_idx);
					} else {
						alert("에디토리얼 상품 삭제처리에 실패했습니다. 삭제하려는 에디토리얼 상품을 확인해주세요.");
					}
				}
			});
		}
...
function openEditorialProductModal() {
		...
		success: function(d) {
			if(d.code == 200){
				...
				if(data != null){
					...
					strDiv += '	<tr>';
					strDiv += '		<td rowspan="2">';
					strDiv += '			<span>썸네일 수량</span>';
					strDiv += '		</td>';
					strDiv += '		<td>';
					strDiv += '			<span>웹</span>';
					strDiv += '		</td>';
					strDiv += '		<td>';
					strDiv += '			<span>' + data.thmb_web_cnt + '개</span>';
					strDiv += '		</td>';
					strDiv += '	</tr>';
					strDiv += '	<tr>';
					strDiv += '		<td>';
					strDiv += '			<span>모바일</span>';
					strDiv += '		</td>';
					strDiv += '		<td>';
					strDiv += '			<span>' + data.thmb_mobile_cnt + '개</span>';
					strDiv += '		</td>';
					strDiv += '	</tr>';
					strDiv += '	<tr>';
					strDiv += '		<td rowspan="2">';
					strDiv += '			<span>컨텐츠 수량</span>';
					strDiv += '		</td>';
					strDiv += '		<td>';
					strDiv += '			<span>웹</span>';
					strDiv += '		</td>';
					strDiv += '		<td>';
					strDiv += '			<span>' + data.cnts_web_cnt + '개</span>';
					strDiv += '		</td>';
					strDiv += '	</tr>';
					strDiv += '	<tr>';
					strDiv += '		<td>';
					...
					strDiv += '		</td>';
					strDiv += '		<td>';
					strDiv += '			<span>' + data.cnts_mobile_cnt + '개</span>';
					strDiv += '		</td>';
					strDiv += '	</tr>';
					strDiv += '</table>';
	...
	confirm('이전의 사진은 모두 삭제 됩니다. 새로 등록하시겠습니까?', function (){
		...
		$.ajax({
			...
			success: function(d) {
				if(d.code == 200){
					closeLoadingWithMask();
					alert('에디토리얼이 등록됬습니다.', getEditorialInfoList());
				}
				else{
					closeLoadingWithMask();
					alert(d.msg);
...
function checkEditorialDisplayNum(obj) {
	var display_num_max = $('.tr_editorial').length;
