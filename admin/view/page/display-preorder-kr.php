<div class="content__card">
	<div class="card__header">
		<div class="flex justify-between">
			<h3>한국몰 프리오더 검색</h3>
			<div class="black-btn" onclick="moveRegistPage('KR')">한국몰 프리오더 등록</div>
		</div>
	...
	<div class="card__footer">
		<div class="footer__btn__wrap">
			...
			<div class="btn__wrap--lg">
				<div class="blue__color__btn" onClick="getPreorderListKr();"><span>검색</span></div>
			...
			<div class="table table__wrap">
				...
				<div class="overflow-x-auto">
					<table>
						...
						<tbody id="preorder_result_table_kr"></tbody>
					</table>
				</div>
			</div>
		</form>
...
function getPreorderListKr(){
	$("#preorder_result_table_kr").html('');
	var strDiv = '';
	strDiv += '<TD class="default_td" colspan="15">';
	strDiv += '    조회 결과가 없습니다';
	strDiv += '</TD>';
	
	$("#preorder_result_table_kr").append(strDiv);
	
	var rows = $('#frm-preorder-filter_kr').find('.rows').val();
	var page = $('#frm-preorder-filter_kr').find('.page').val();
	
	get_contents($("#frm-preorder-filter_kr"),{
		pageObj : $("#frm-preorder-list_kr").find(".paging"),
		html : function(d) {
			if (d.length > 0) {
				$("#preorder_result_table_kr").html('');
			}
