...
<body>
	...
	$r_url = getUrlParamter($_SERVER['REQUEST_URI'], 'r_url');
					...
					<div class="form-group margin-top-20">
						<div class="input-icon">
							...
							<input class="form-control placeholder-no-fix" type="text" autocomplete="off"
								placeholder="이메일주소" name="email" required>
			...
			<div class="con" id="loginbox">
				...
				<form name="frm" id="frm" method="post" onSubmit="login();return false;">
					...
					<div class="form-group">
						<div class="input-icon">
							<i class="xi-user"></i>
							<input class="form-control placeholder-no-fix" type="text" minlength="4" autocomplete="off"
								placeholder="아이디" name="id" value="<?=$id?>" required>
							<span class="-describe"></span>
						</div>
					</div>
					<div class="form-group">
						<div class="input-icon">
							<i class="xi-lock"></i>
							<input class="form-control placeholder-no-fix" type="password" minlength="4" maxlength="20"
								autocomplete="off" placeholder="비밀번호" name="pw" value="<?= $pwd ?>" required>
							<span class="-describe"></span>
						</div>
					</div>
					<div class="form-actions">
						<label class="checkbox">
							<input type="checkbox" name="remember" value="1" <?php if ($remember)
								echo "checked"; ?>>
							<i class="xi-check"></i>
							<span>비밀번호 기억</span>
						</label>
				...
				<div class="forget-password">
					...
					<p><a id="forget-password" onclick="$('#loginbox').hide();$('#pwbox').show();">여기</a>에서 임시 비밀번호를 새로
						발급받으세요</p>
				</div>
				<div class="bg"></div>
			</div>
...
<script>
	$(document).ready(function () {
		$('#login-intro .xi-lock').on('click', function () {
			let pw_input = $(this).prev();
			pw_input.toggleClass('active');
			if (pw_input.hasClass('active')) {
