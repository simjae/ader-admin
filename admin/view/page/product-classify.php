...
<div class="main-wrap">
	<div class="product-tree">
		<div class="content__card" style="padding: 11px 0 20px 0;">
			<div class="card__header">
				<div class="flex justify-between" style="padding:0 20px;">
					<div class="filter-wrap" style="margin-bottom:20px">
						<button class="classify_tab_btn tap__button" tab_num="01" style="background-color: #000;color: #fff;font-weight: 500;" onClick="classifyTabBtnClick(this);">오더시트 분류</button>
						<button class="classify_tab_btn tap__button" tab_num="02" onClick="classifyTabBtnClick(this);">독립몰 상품 분류</button>
					</div>
				</div>
				<div class="flex justify-between" style="padding:0 20px;">
			...
			<div id="js--tree--ordersheet"></div>
			<div id="js--tree--product" style="display:none"></div>
		</div>
			...
			<div class="content__card">
				<form id="frm-filter" action="">
					...
					<div class="card__body">
						...
						<div class="table table__wrap">
							<TABLE id="ordersheet_list_table">
								<THEAD>
									<TR>
										<TH style="width:3%;">
											<div class="cb__color">
								...
								<TBODY id="result_ordersheet_table">
								</TBODY>
							</TABLE>
							<TABLE id="product_list_table" style="display:none">
								<THEAD>
									<TR>
										<TH style="width:3%;">
											<div class="cb__color">
...
$(document).ready(function() {
	...
	.on('create_node.jstree', function (e, data) {
		var ref = $('#js--tree--product').jstree(true);
		...
		.success(function (d) {
			insertLog("상품관리 > 상품 분류 관리 > 카테고리 생성", "생성된 카테고리 : " + data.node.text, null);
	...
	.on('delete_node.jstree', function (e, data) {
		...
		.success(function(){
			insertLog("상품관리 > 상품 분류 관리 > 카테고리 삭제", "삭제된 카테고리 : " + data.node.text, null);
		})
		.fail(function () {
			data.instance.refresh();
		});
	...
	.on('rename_node.jstree', function (e, data) {
		...
		.success(function (d) {
			insertLog("상품관리 > 상품 분류 관리 > 카테고리 이름변경", "변경된 카테고리 명: " + data.old + " -> " + data.text, null);
		})
		.fail(function () {
			data.instance.refresh();
		});
	...
	.on('changed.jstree', function (e, data) {
		...
		if(data && data.selected && data.selected.length) {
			...
			if(sel_node.length != 0){
				...
				$('#category_list').text("["+current_cate+"] 독립몰 상품 분류 관리");
	...
	.on('create_node.jstree', function (e, data) {
		var ref = $('#js--tree--ordersheet').jstree(true);
		var parant_node = ref.get_node(data.parent);
	...
	$('#tree--search').on('change', function() {
		...
		switch(tab_num){
			case '01':
				jstree_div = $('#js--tree--ordersheet');
				break;
			case '02':
				jstree_div = $('#js--tree--product');
				break;
		}
		jstree_div.jstree(true).search(searchVal);
	});
});
...
function exCreate() {
	...
	switch(tab_num){
		case '01':
			ref = $('#js--tree--ordersheet').jstree(true);
			break;
		case '02':
			ref = $('#js--tree--product').jstree(true);
			break;
	}
...
function exMove() {
	...
	insertLog("상품관리 > 상품 분류 관리 > 상품 분류 설정", "분류 이동 저장", null);
...
function classifyTabBtnClick(obj) {	
	...
	switch(tab_num){
		case '01':
			title = '독립몰 상품 분류 관리';
			$('#js--tree--ordersheet').css('display', 'block');
			$('#js--tree--ordersheet').jstree('close_all');
			$('#js--tree--product').css('display', 'none');
			break;
		case '02':
			title = '오더시트 분류 관리';
			$('#js--tree--product').css('display', 'block');
			$('#js--tree--product').jstree('close_all');
			$('#js--tree--ordersheet').css('display', 'none');
			break;
	}
	...
	$('.classify_tab_btn').not($(obj)).css('background-color','#ffffff');
	$('.classify_tab_btn').not($(obj)).css('color','#000000');
...
function setJsTree(tab_num){
	var sel_jstree = '';
	var title_str = '';
	switch(tab_num){
		case '01':
			sel_jstree = $('#js--tree--ordersheet').jstree(true);
			title_str = '오더시트 분류 관리';
			break;
		case '02':
			sel_jstree = $('#js--tree--product').jstree(true);
			title_str = '독립몰 상품 분류 관리';
			break;
	}

	var sel_id = sel_jstree.get_selected();
	var sel_node = sel_jstree.get_node(sel_id);

	if(sel_node.length != 0){
		$("input[name='md_category_node']").val('');
		$("input[name='md_category_depth']").val('');

		$('#cate_id').val(sel_node.id);
		$('.desciption__table').find('th').eq(1).text(sel_node.text);
		$('.desciption__table').find('input').eq(0).val(sel_node.text);
		$('.desciption__table').find('textarea').eq(0).val(sel_node.original.description);
		$('#category_list').text("["+sel_node.text+"] " + title_str);
...
function getProdTabInfo(){
	...
	switch(tab_num){
		case '01':
			$('#frm-list').attr('action','pm/ordersheet/list/get');
			result_table = $("#result_ordersheet_table");
			colspan_cnt = 9;
			break;
		case '02':
			$('#frm-list').attr('action','product/list/get');
			result_table = $("#result_product_table");
			colspan_cnt = 10;
			break;
	}
	result_table.html('');
	...
	var strDiv = '';
	strDiv += '<TD class="default_td" colspan="' + colspan_cnt + '">';
	strDiv += '    조회 결과가 없습니다';
	strDiv += '</TD>';
				...
				switch(tab_num){
					case '01':
						...
						var strDiv = "";
							...
							$("#ordersheet_list_table").css('display','block');
							$("#product_list_table").css('display','none');
						break;
					case '02':
						var strDiv = "";
							strDiv += '<tr>';
							...
							$("#ordersheet_list_table").css('display','none');
							$("#product_list_table").css('display','block');
						break;
				}
				result_table.append(strDiv);
			});
