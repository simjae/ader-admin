			...
			<div class="overflow-x-auto">
				<table>
					...
					<tbody>
						<tr>
							<td>
								<div type="button" class="btn" onclick="addSubMaterialDelivery()">생성</div>
							</td>
							<td>
		...
		<div class="card__body">
			<div class="content__wrap grid__half">
				<div class="half__box__wrap">
					<div class="content__title">배송부자재 타입</div>
				...
				<div class="half__box__wrap">
					<div class="content__title">배송부자재 이름</div>
					<div class="content__row">
			...
			<div class="content__wrap grid__half">
				<div class="half__box__wrap">
					<div class="content__title">배송부자재 코드</div>
					<div class="content__row">
		...
		<div class="card__footer">
			<div class="footer__btn__wrap" style="grid: none;">
				<div class="btn__wrap--lg">
					<div  class="blue__color__btn" onClick="getSubMaterialDeliveryList()"><span>검색</span></div>
					<div class="defult__color__btn" onClick="init_fileter('frm-filter','getSubMaterialDeliveryList');"><span>초기화</span></div>
				</div>
			</div>
		</div>
	</form>
...
<div class="content__card" style="height: 90vh;">
	...
	<form id="frm-list-delivery">
		<div class="info__wrap " style="justify-content:space-between; align-items: center;">
			...
			<div class="content__row">
				<div class="btn" onclick="deliverySMTotalUpdate()" style="color:#ffffff;background-color:#ffa500">일괄 수정</div>
...
function getSubMaterialDeliveryList(){
	let frm = $('#frm-filter');
...
function addSubMaterialDelivery(){
	confirm('부자재를 등록하시겠습니까?',function(){
		...
		$.ajax({
			...
			processData: false,
			error: function() {
				alert('배송부자재 등록처리중 오류가 발생했습니다.');
			},
			success: function(d) {
				if (d.code == 200) {
					getSubMaterialDeliveryList();
					alert('선택한 배송부자재 정보가 등록되었습니다.');
				} else {
					alert(d.msg);
				}
			}
...
function putSubMaterialDelivery(obj) {
	...
	confirm(
		...
		function() {
			$.ajax({
				...
				error: function() {
					alert('배송부자재 정보 수정처리중 오류가 발생했습니다.');
				},
				success: function(d) {
					if (d.code == 200) {
						getSubMaterialDeliveryList();
						alert('선택한 배송부자재 정보가 수정되었습니다.');
					} else {
						alert(d.msg);
					}
				}
...
function deliverySMTotalUpdate(){
	confirm('현재 기입된 배송부자재 정보를 일괄 수정하시겠습니까?', function(){
        let formData = new FormData();
        formData = $('#frm-list-delivery').serializeObject();
        ...
        if(formData['sub_material_code_list[]'] != null && formData['sub_material_code_list[]'].length > 0){
            ...
            $.ajax({
                ...
                url: config.api + "pcs/ordersheet/td/sub_material/put",
                error: function() {
                    alert('배송부자재 일괄편집 처리에 실패했습니다.');
                },
                success: function(d) {
                    if(d.code == 200) {
                        alert('배송부자재 일괄편집 처리에 성공했습니다.');
                        getSubMaterialDeliveryList();
                    }
                    else{
                        alert(d.msg);
            ...
            alert('수정할 수 있는 배송부자재가 존재하지 않습니다.');
            return false;
        } 
    });
}
...
function updateImage(obj, form_id){
	if(checkImageType(obj)){
		...
		$.ajax({
			...
			processData: false,
			error: function() {
				alert('배송부자재 이미지수정 처리중 오류가 발생했습니다.');
			},
			success: function(d) {
				if (d.code == 200) {
					getSubMaterialDeliveryList();
					alert('선택한 배송부자재 이미지가 수정되었습니다.');
				} else {
					alert(d.msg);
				}
			}
...
function deleteSubMaterialDelivery(sub_material_idx) {
	confirm(
		'선택한 배송부자재 정보를 삭제하시겠습니까?',
		function() {
			$.ajax({
				type: "post",
				...
				dataType: "json",
				error: function() {
					alert('배송부자재 정보 삭제처리중 오류가 발생했습니다.');
				},
				success: function(d) {
					if (d.code == 200) {
						alert('선택한 배송부자재 정보가 삭제되었습니다.');
						getSubMaterialDeliveryList();
					} else{
						alert(d.msg);
					}
...
function openDeliverySubMaterialUseProductModal(idx) {
