		...
		<div class="card__body">
			<div class="info__wrap " style="justify-content:space-between; align-items: center;">
				<div class="body__info--count">
					...
					총 샘플 목록 수 <font class="cnt_total info__count" >0</font>개 / 검색결과 <font class="cnt_result info__count" >0</font>개
				</div>
    ...
    <div class="card__body">
						...
						<TABLE id="insert_table_sample_info">
							<THEAD>
								<TR>
									...
									<TH style="width:15%">샘플 납기 예정일</TH>
									<TH style="width:15%">샘플 납기 수량</TH>
									<TH style="width:40%">메모</TH>
								</TR>
							</THEAD>
							<TBODY id="result_update_table">
								<TR>
									<TD>
										<span id="product_code_td"><span>
                                    </TD>
									<TD>
										<input type="hidden" name="sample_idx" value="">
		...
		<div class="flex justify-center">
			<button type="button"
				...
				onClick="confirm('샘플을 수정하시겠습니까?.','sampleUpdate()');">샘플수정</button>
		</div>
    </div>
</div>
<script>
...
function urlParsing(){
	...
	if(idx >= 0){
		...
		for(var i = 0; i < data_arr.length; i++){
			...
			switch(param_obj[0]){
				case 'sample_idx':
					$('input[name=sample_idx]').val(param_obj[1]);
					break;
				case 'product_code':
...
function setSampleInput(){
    var sample_idx = $('input[name=sample_idx]').val();
	...
	$.ajax({
		...
		dataType: "json",
		url: config.api + "pcs/sample/list/get",
		error: function() {
		...
		success: function(d) {
			if(d.code == 200) {
				...
				if (data.length > 0) {
					...
					$('input[name=memo]').val(data[0].memo);
				}
			}
		}
	});
...
function getSampleModalTotal() {
	...
	var product_code = $('#frm-list').find('input[name=product_code]').val();
...
function sampleUpdate() {
	var manufacturer = $('#result_update_table').find('input[name=manufacturer]').val();
	if(manufacturer.length == 0){
		alert('등록하려는 샘플의 제조사를 입력해주세요');
		return false;
	}

	var due_date = $('#result_update_table').find('input[name=due_date]').val();
	...
	var product_qty = $('#result_update_table').find('input[name=product_qty]').val();
