...
<div class="content__card">
	<form id="frm-filter" action="pm/factory/list/get">
		...
		<div class="card__body">
			...
			<div class="content__wrap">
				<div class="content__title">제조사 납기 예정일</div>
				<div class="content__row">
			...
			<div class="content__wrap">
                <div class="content__title">제조사 납기 수량</div>
                <div class="content__row">
		...
		<div class="card__footer">
			<div class="footer__btn__wrap" style="grid: none;">
                ...
                <div  class="blue__color__btn" onClick="getFactoryTabInfo()"><span>검색</span></div>
					<div class="defult__color__btn" onClick="init_fileter('frm-filter','getFactoryTabInfo()');"><span>초기화</span></div>
				</div>
			</div>
		</div> 
	</form>
		...
		<div class="table__wrap table">
			<div class="table__filter">
				<div class="filrer__wrap">
					<div style="width: 50px;" class="filter__btn" action_type="prod_copy" onclick="resetFactoryTotal();">초기화</div>
				</div>                                
			</div>
			<TABLE>
				<THEAD>
					<TR>
						...
						<TH style="width:25%;">공장수주 생산공장</TH>
						<TH style="width:20%;">공장수주 납기일</TH>
						<TH style="width:20;">공장수주 납기 총수량</TH>
					</TR>
				</THEAD>
		...
		<div class="card__body">
			<div class="info__wrap " style="justify-content:space-between; align-items: center;">
				<div class="body__info--count">
					<div class="drive--left"></div>
					총 공장수주 목록 수 <font class="list_cnt cnt_total info__count" >0</font>개 / 검색결과 <font class="list_cnt cnt_result info__count" >0</font>개
				</div>
				...
				<div class="content__row">
					<select style="width:163px;float:right;margin-right:10px;" onChange="orderChange(this);">
                        ...
                        <option value="MANUFACTURER|DESC">제조사 역순</option>
						<option value="MANUFACTURER|ASC">제조사 순</option>
					</select>
			...
			<div class="table table__wrap list">
				<div class="table__filter">
					<div class="filrer__wrap">
						<div style="width: 140px;" class="filter__btn" action_type="factory_delete" onclick="factoryActionCheck(this)">삭제</div>
				...
				strDiv = `
					<tr > 
						<td>
							<div class="cb__color">
								<label>
									<input type="checkbox" class="select" name="factory_idx" value="${row.factory_idx}">
									<span></span>
								</label>
							</div>
						</td>
						<td>${row.num}</td>
						<td><button type="button" class="white_btn" onclick="location.href='/product/sample/update?factory_idx=${row.factory_idx}&product_code=${row.product_code}'">편집</button></td>
						<td search-keyword="manufacturer" search-value="${row.manufacturer}" onclick="getFactoryTotal(this)" style="cursor:pointer">${row.manufacturer}</td>
						<td search-keyword="product_code" search-value="${row.product_code}" onclick="getFactoryTotal(this)" style="cursor:pointer">${row.product_code}</td>
						<td search-keyword="product_code" search-value="${row.product_code}" onclick="getFactoryTotal(this)" style="cursor:pointer">
							<div class="product__img__wrap">
								<div class="product__img" style=""></div>
								<div>
...
function factoryActionCheck(obj) {
	...
	if (select_idx.length == 0) {
		alert(action_name + ' 처리 할 공장수주를 선택해주세요.');
	} else {
		var cnt = 0;
		...
		confirm('선택한 공장수주를 ' + action_name + ' 하시겠습니까?','factoryAction(' + select_idx.length + ')');
	}
    
}

function factoryAction(len){
	var api_str = '';
	...
	$.ajax({
		...
		data: formData,
		dataType: "json",
		url: config.api + "pm/factory/delete",
		error: function() {
		...
		success: function(d) {
			if(d.code == 200) {
				...
				insertLog("상품관리 > 공장수주 등록 ", action_name, len);
				getFactoryTabInfo();
			}
		}
	});
...
function excelDownload() {
	if ($('#result_table').find('.default_td').length > 0) {
		...
		sheet_name = "공장수주목록";
		file_name = "공장수주목록_" + file_date;
		insertLog("상품관리 > 공장수주목록", "엑셀다운로드 : "+file_name+"xlsx", 1);
