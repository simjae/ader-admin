...
<div class="content__card">
	<form id="frm-standby-update" action="order/standby/put">
		...
		$param = getUrlParamter($page_url, 'param');
		?>
		<input type="hidden" id="param" value="<?=$param?>">
		<input type="hidden" name="country">
		<input type="hidden" name="standby_idx">
					...
					<div class="content__row">
						<div class="content__date__wrap">
							<div class="content__date__picker">
								...
								<select id="purchase_start_hour" class="time__select hour" date_type="purchase" onChange="dateParamChange(this);"></select>
								<span>&nbsp;시
							</div>
						</div>
					</div>
						...
						<div class="content__date__wrap">
							<div class="content__date__picker">
								<input class="date_param" type="date" id="purchase_end_date"
									class="margin-bottom-6" placeholder="From" readonly style="width:150px;"
									date_type="purchase" onChange="dateParamChange(this);">
								<select id="purchase_end_hour" class="time__select hour" date_type="purchase" onChange="dateParamChange(this);"></select>
								<span>&nbsp;시
							</div>
						</div>
					</div>
		...
		<div class="card__footer">
			<div class="footer__btn__wrap">
				...
				<div class="btn__wrap--lg">
					<div class="blue__color__btn" onClick="putStandby('KR');"><span>스탠바이 편집</span></div>
					<div class="defult__color__btn" style="cursor:pointer;" onclick="location.href='/display/standby'"><span>이전페이지로 돌아가기</span></div>
				</div>
			</div>
		</div>
	</form>
...
$(document).ready(function () {
	...
	$('input[name="country"]').val(param_arr[0]);
	$('input[name="standby_idx"]').val(param_arr[1]);
	timeSelectSet();

	$.ajax({
		url: config.api + "order/standby/get",
		type: "post",
		data: {
			standby_idx: $('input[name="standby_idx"]').val(),
				...
				$.ajax({
					...
					success: function(d) {
						...
						if (code == 200) {
							...
							$('#standby_product_table').append(strDiv);
...
function putStandby(country){
	confirm('스탠바이를 편집하시겠습니까?', function(){

		var formData = new FormData();
		formData = $('#frm-standby-update').serializeObject();
		...
		$.ajax({
			...
			dataType: "json",
			error: function() {
				alert('스탠바이 편집 처리중 오류가 발생했습니다.');
			},
			success: function(d) {
				let code = d.code;
				if (code == 200) {
					alert('스탠바이 편집이 완료되었습니다.');
				} else {
					alert('스탠바이 편집에 실패했습니다.');
					return false;
				}
			}
		});
