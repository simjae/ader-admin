	...
	<div class="card__footer">
		<div class="footer__btn__wrap">
			...
			<div class="btn__wrap--lg">
				<div class="blue__color__btn" onClick="getDrawListKr();"><span>검색</span></div>
...
function getDrawListKr(){
	$("#draw_result_table_kr").html('');
	var strDiv = '';
	strDiv += '<TD class="default_td" colspan="15">';
	strDiv += '    조회 결과가 없습니다';
	strDiv += '</TD>';
	
	$("#draw_result_table_kr").append(strDiv);
	
	var rows = $('#frm-draw-filter_kr').find('.rows').val();
	var page = $('#frm-draw-filter_kr').find('.page').val();
	
	get_contents($("#frm-draw-filter_kr"),{
		pageObj : $("#frm-draw-list_kr").find(".paging"),
		html : function(d) {
			if (d.length > 0) {
				$("#draw_result_table_kr").html('');
			}
			...
			d.forEach(function(row) {
				...
				strDiv = `
					<tr>
						<td rowspan="${rowspan_num}">
							<div class="cb__color">
								<label>
									<input type="checkbox" class="select" name="draw_idx[]" value="${row.draw_idx}" ">
									<span></span>
								</label>
							</div>
						</td>
						<td rowspan="${rowspan_num}">        
							<div class="btn" action_type="up" display_num="${row.display_num}" idx=${row.draw_idx} onclick="displayNumCheck(this)">            
								<i class="xi-angle-up"></i>            
								<span class="tooltip top">위로</span>        
							</div>        
							<div class="btn" action_type="down" display_num="${row.display_num}" idx=${row.draw_idx} onclick="displayNumCheck(this)">          
				...
				row.qty_info.forEach(function(qty_row, index){
					optionDiv = `
						...
						<td>0</td>
						<td>
							<div class="btn" onclick="viewDrawEntry(${row.draw_idx}, ${qty_row.option_idx},'KR')">조회</div>
						</td>
					`;
					if(index != 0){
				...
				strDiv += `
						<td rowspan="${rowspan_num}">
							<div class="btn" onclick="viewDrawEntry(${row.draw_idx}, 'null', 'KR')">전체조회</div>
						</td>
						...
						<td rowspan="${rowspan_num}">
							<div class="btn" onclick="prizeDraw('KR', ${row.draw_idx})">당첨</div>
						</td>
						<td rowspan="${rowspan_num}">
							<div class="btn" onclick="moveUpdatePage('KR', ${row.draw_idx})">편집</div>
						</td>
						<td rowspan="${rowspan_num}">
							<div class="btn" sel_idx="${row.draw_idx}" onclick="toggleDisplayFlg(this)">${display_str}</div>
						</td>
					</tr>
				`;
