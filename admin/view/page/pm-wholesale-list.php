...
<div class="content__card">
	<form id="frm-filter" action="pm/wholesale/list/get">
		...
		<div class="flex justify-between" style="gap:20px;">
			...
			<div>
				<button type="button" style="width:120px;height:30px;border:1px solid #000000;background-color:#ffffff;color:#000000;margin-right:10px;cursor:pointer;" onclick="location.href='/pm/wholesale/buyer/regist'">홀세일 등록</button>
			</div>
		</div>
			...
			d.forEach(function(row) {
				...
				strDiv = `
					<tr > 
						...
						<td>${row.num}</td>
						<td><button type="button" class="white_btn" onclick="location.href='/pm/wholesale/update?wholesale_idx=${row.wholesale_idx}&product_code=${row.product_code}'">편집</button></td>
...
function wholesaleAction(len){
	...
	$.ajax({
		...
		data: formData,
		dataType: "json",
		url: config.api + "pm/wholesale/delete",
		error: function() {
