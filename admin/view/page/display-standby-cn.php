	...
	<div class="card__footer">
		<div class="footer__btn__wrap">
			...
			<div class="btn__wrap--lg">
				<div class="blue__color__btn" onClick="getStandbyListCn();"><span>검색</span></div>
						...
						<thead>
							<tr>
								<th>
									<div class="cb__color">
										<label>
											<input type="checkbox" class="select" name="draw_idx[]" value="" onclick="selectAllClick(this,'cn')">
											<span></span>
										</label>
									</div>
								</th>
...
function getStandbyListCn(){
	$("#standby_result_table_cn").html('');
	var strDiv = '';
	strDiv += '<TD class="default_td" colspan="15">';
	strDiv += '    조회 결과가 없습니다';
	strDiv += '</TD>';
	
	$("#standby_result_table_cn").append(strDiv);
	
	var rows = $('#frm-standby-filter_cn').find('.rows').val();
	var page = $('#frm-standby-filter_cn').find('.page').val();
	
	get_contents($("#frm-standby-filter_cn"),{
		pageObj : $("#frm-standby-list_cn").find(".paging"),
		html : function(d) {
			if (d.length > 0) {
				$("#standby_result_table_cn").html('');
			}
			...
			d.forEach(function(row) {
				...
				row.qty_info.forEach(function(qty_row, index){
					optionDiv = `
						...
						<td>0</td>
						<td>
							<div class="btn" onclick="viewStandbyEntry(${row.standby_idx}, ${qty_row.option_idx},'CN')">조회</div>
						</td>
					`;
					if(index != 0){
				...
				strDiv += `
						<td rowspan="${rowspan_num}">
							<div class="btn" onclick="viewStandbyEntry(${row.standby_idx}, 'null', 'CN')">전체조회</div>
						</td>
						...
						<td rowspan="${rowspan_num}">
							<div class="btn" onclick="moveUpdatePage('CN', ${row.standby_idx})">편집</div>
						</td>
						<td rowspan="${rowspan_num}">
