	...
	<div class="card__footer">
		<div class="footer__btn__wrap">
			...
			<div class="btn__wrap--lg">
				<div class="blue__color__btn" onClick="getPreorderListCn();"><span>검색</span></div>
				<div class="defult__color__btn" onClick="resetMenu('CN');"><span>초기화</span></div>
			</div>
		</div>
	</div>
</div>
			...
			<div class="info__wrap " style="justify-content:space-between; align-items: center;">
				...
				<div class="content__row">
					<select style="width:163px;float:right;margin-right:10px;" onChange="orderChange(this);">
						...
						<option value="SALES_PRICE_CN|DESC">판매가(힌국몰) 역순</option>
						<option value="SALES_PRICE_CN|ASC">판매가(힌국몰) 순</option>
...
function getPreorderListCn(){
	$("#preorder_result_table_cn").html('');
	var strDiv = '';
	strDiv += '<TD class="default_td" colspan="15">';
	strDiv += '    조회 결과가 없습니다';
	strDiv += '</TD>';
	
	$("#preorder_result_table_cn").append(strDiv);
	
	var rows = $('#frm-preorder-filter_cn').find('.rows').val();
	var page = $('#frm-preorder-filter_cn').find('.page').val();
	
	get_contents($("#frm-preorder-filter_cn"),{
		pageObj : $("#frm-preorder-list_cn").find(".paging"),
		html : function(d) {
			if (d.length > 0) {
				$("#preorder_result_table_cn").html('');
			}
			...
			d.forEach(function(row) {
						...
						<td rowspan="${rowspan_num}">
							...
							<div class="product__img__wrap">
								...
								<span>
									...
									<p>${parseInt(row.sales_price).toLocaleString('ko-CN')} ₩</p><br>
				...
				row.qty_info.forEach(function(qty_row, index){
					optionDiv = `
						...
						<td>0</td>
						<td>
							<div class="btn" onclick="viewPreorderEntry(${row.preorder_idx}, ${qty_row.option_idx},'CN')">조회</div>
						</td>
					`;
					if(index != 0){
				...
				strDiv += `
						<td rowspan="${rowspan_num}">
							<div class="btn" onclick="viewPreorderEntry(${row.preorder_idx}, 'null', 'CN')">전체조회</div>
						</td>
						<td rowspan="${rowspan_num}">${row.sales_price}원</td>
						<td rowspan="${rowspan_num}">${row.entry_start_date}<br>- ${row.entry_end_date}</td>
						<td rowspan="${rowspan_num}">
							<div class="btn" onclick="moveUpdatePage('CN', ${row.preorder_idx})">편집</div>
						</td>
						<td rowspan="${rowspan_num}">
