...
<div class="content__card">
	<form id="frm-add" action="display/posting/add">
		<input id="posting_type" type="hidden" value="collaboration" style="width:80%;" placeholder="" name="posting_type">
		...
		<div class="card__footer">
			<div class="footer__btn__wrap" style="grid: none;">
				<div class="btn__wrap--lg">
					<div  onclick="postingCollaborationRegist();"  class="blue__color__btn"><span>저장</span></div>
...
function postingCollaborationRegist(){
	...
	confirm('콜라보레이션 페이지를 등록하시겠습니까?',function() {
		$.ajax({
			type: "post",
			data: formData,
			dataType: "json",
			...
			error: function() {
				alert("콜라보레이션 페이지 등록 처리에 실패했습니다.");
			},
			success: function(d) {
				if(d.code == 200) {
					insertLog("전시관리 > 게시물 관리", "새 콜라보레이션 페이지 등록", 1);
					alert("콜라보레이션 페이지 등록 처리에 성공했습니다.");
