<div class="content__card">
	<div class="card__header">
		<div class="flex justify-between">
			<h3>룩북 리스트</h3>
			<div class="black-btn" onClick="location.href='/display/posting/lookbook/regist'">추가하기</div>
		</div>
		<div class="drive--x"></div>
	</div>
	
	<div class="flex" style="gap:50px;margin:20px 0;">
		<div class="category__tab lookbook_country_btn" country="ALL" style="color:#140f82;border-bottom: 3px solid #140f82;text-align: center;cursor: pointer;" onClick="lookbookCountryTabClick(this);">전체</div>
		<div class="category__tab lookbook_country_btn" country="KR" style="height:30px;color:#707070;text-align:center;cursor:pointer;" onClick="lookbookCountryTabClick(this);">한국몰</div>
		<div class="category__tab lookbook_country_btn" country="EN" style="height:30px;color:#707070;text-align:center;cursor:pointer;" onClick="lookbookCountryTabClick(this);">영문몰</div>
		<div class="category__tab lookbook_country_btn" country="CN" style="height:30px;color:#707070;text-align:center;cursor:pointer;" onClick="lookbookCountryTabClick(this);">중문몰</div>
	</div>
	
	<div class="card__body">
		<form id="frm-05-filter" action="display/posting/get">
			<input type="hidden" class="tab_num" name="tab_num" value="05">
		...
		<form id="frm-05-list">
			<div class="info__wrap " style="justify-content:space-between; align-items: center;">
				<div class="body__info--count">
					...
					총 <font id="cnt_05_total" class="info__count">0</font>개 
				</div>
			...
			<div class="table table__wrap">
				...
				<TABLE>
					...
					<TBODY id="result_05_table">
					</TBODY>
				</TABLE>
			</div>
			
			<div class="padding__wrap">
				<input type="hidden" class="total_cnt" tab_num="05" value="0" onChange="setPaging(this);">
...
function lookbookCountryTabClick(obj) {
	var country = $(obj).attr('country');
	if (country != null) {
		$('#frm-05-filter').find('input[name="country"]').eq(0).val(country);
		$('.lookbook_country_btn').not($(obj)).css('color','#707070');
		$('.lookbook_country_btn').not($(obj)).css('border-bottom','none');
...
function getPostingLookbookInfo() {
	$("#result_05_table").html('');
	...
	$("#result_05_table").append(strDiv);
	
	var rows = $('#frm-05-filter').find('.rows').val();
	var page = $('#frm-05-filter').find('.page').val();

	get_contents($('#frm-05-filter'),{
		pageObj : $('#frm-05-list').find(".paging"),
		html : function(d) {
			if (d.length > 0) {
				$("#result_05_table").html('');
			}
			d.forEach(function(row) {
				...
				strDiv += '    <TD>';
				strDiv += '        <button type="button" page_idx="' + row.idx + '" page_url="' + row.page_url + '" style="font-size:0.5rem;width:60px;height:30px;border:1px solid;background-color:#ffffff;" onClick="location.href=\'/display/posting/lookbook/page?page_idx=' + row.idx + '\';">페이지 편집</button>';
				strDiv += '    </TD>';
				strDiv += '    <TD>';
				...
				$('#frm-05-list').find('input[name="selectAll"]').prop('checked', false);
				$("#result_05_table").append(strDiv);
			});
		},
	},rows,page);
