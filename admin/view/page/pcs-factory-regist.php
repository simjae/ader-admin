		...
		<div class="card__body">
			<div class="info__wrap " style="justify-content:space-between; align-items: center;">
				<div class="body__info--count">
					...
					총 공장수주 목록 수 <font class="cnt_result info__count" >0</font>개
				</div>
    ...
    <div class="card__body">
		...
		<div class="flex justify-center">
			<button type="button"
				...
				onClick="confirm('공장수주를 등록하시겠습니까?.','factoryInsert()');">공장수주 등록</button>
		</div>
    </div>
</div>
<script>
...
function factoryInsert() {
	var manufacturer = $('#insert_table_factory_info').find('input[name=manufacturer]').val();
	if(manufacturer.length == 0){
		alert('등록하려는 공장수주의 제조사를 입력해주세요');
		return false;
	}

	var due_date = $('#insert_table_factory_info').find('input[name=due_date]').val();
	...
	var product_qty = $('#insert_table_factory_info').find('input[name=product_qty]').val();
	...
	$.ajax({
		...
		data: formData,
		dataType: "json",
		url: config.api + "pcs/factory/add",
		cache: false,
		contentType: false,
		processData: false,
		error: function() {
