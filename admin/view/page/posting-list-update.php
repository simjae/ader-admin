<div class="content__card">
	<form id="frm-put" action="posting/list/put">
		<?php
		...
		<div class="card__footer">
			<div class="footer__btn__wrap">
				...
				<div class="btn__wrap--lg">
					<div  class="blue__color__btn" onClick="checkPostingList();"><span>수정</span></div>
...
function putPostingList() {
	var form = $("#frm-put")[0];
	var formData = new FormData(form);

	confirm("게시물 리스트를 수정하시겠습니까?",function() {
		$.ajax({
			type: "post",
			data: formData,
			dataType: "json",
			url: config.api + "posting/list/put",
			cache: false,
			contentType: false,
			processData: false,
			error: function() {
				alert("게시물 리스트 수정 처리에 실패했습니다.");
			},
			success: function(d) {
				if(d.code == 200) {
					confirm("게시물 리스트 수정 처리에 성공했습니다.",function pageLocation() {
...
function getPostingList() {
	let list_idx = $('#list_idx').val();
	
	$.ajax({
		url: config.api + "posting/list/get",
		type: "post",
		data: {
			'list_idx': list_idx,
		},
		dataType: "json",
		error: function() {
			alert('게시물 리스트 조회처리중 오류가 발생했습니다.');
		},
		success: function(d) {
			let code = d.code;
			if (code == 200) {
				...
				if (data != null) {
					data.forEach(function(row) {
						...
						$('#list_title').val(row.list_title);
						$('#list_memo').val(row.list_memo);
						...
						$('#display_start_date').val(row.list_from_date);
						$('#display_start_h').val(row.list_from_h);
						$('#display_start_m').val(row.list_from_m);
						
						$('#display_end_date').val(row.list_to_date);
						$('#display_end_h').val(row.list_to_h);
						$('#display_end_m').val(row.list_to_m);
						
						if (row.page_idx != null) {
							$('#page_idx').val(row.page_idx);
							...
							strDiv += '        </div>';
							strDiv += '    </TD>';
							strDiv += '    <TD>' + row.posting_country + '</TD>';
				...
				alert('게시물 리스트 조회처리에 실패했습니다. 조회하려는 게시물 스토리를 확인해주세요.');
			}
		}
	});
}
