...
<div class="content__card">
	<form id="frm-list" action="pcs/factory/list/get">
		...
		<div class="card__body">
			<div class="info__wrap " style="justify-content:space-between; align-items: center;">
				<div class="body__info--count">
					...
					총 공장수주 목록 수 <font class="cnt_total info__count" >0</font>개 / 검색결과 <font class="cnt_result info__count" >0</font>개
				</div>
...
function urlParsing(){
	...
	if(idx >= 0){
		...
		for(var i = 0; i < data_arr.length; i++){
			...
			switch(param_obj[0]){
				case 'factory_idx':
					$('input[name=factory_idx]').val(param_obj[1]);
					break;
				case 'product_code':
...
function setfactoryInput(){
	...
	$.ajax({
		...
		dataType: "json",
		url: config.api + "pcs/factory/list/get",
		error: function() {
...
function factoryUpdate() {
	...
	$.ajax({
		...
		data: formData,
		dataType: "json",
		url: config.api + "pcs/factory/put",
		cache: false,
		contentType: false,
		processData: false,
		error: function() {
		...
		success: function(d) {
			if(d.code == 200) {
				alert('공장수주가 정상적으로 작성되었습니다.',function pageLocation() {
					location.href = '/pcs/factory/list';
				});
			}
		}
	});
