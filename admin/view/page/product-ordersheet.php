		...
		<div class="card__body">
			<div claszs="body__info--count" style="display: block;margin:20px 0;">
				...
				<div class="flex justify-between" style="gap:20px;">
					...
					<div>
						<button type="button" style="width:120px;height:30px;border:1px solid #000000;background-color:#ffffff;color:#000000;margin-right:10px;cursor:pointer;" onclick="location.href='/product/ordersheet/register/md'">오더시트 등록</button>
			...
			<div class="content__wrap">
				...
				<div class="content__row">
					<input type="text" name="style_code" value="" style="width:15%">
				</div>
			</div>
			<div class="content__wrap">
				<div class="content__title">색상코드</div>
				<div class="content__row">
					<input type="text" name="color_code" value="" style="width:15%">
				</div>
			</div>
			<div class="content__wrap">
				<div class="content__title">상품코드</div>
				<div class="content__row">
					<input type="text" name="product_code" value="" style="width:15%">
				</div>
			</div>
			<div class="content__wrap">
				<div class="content__title">상품명</div>
				<div class="content__row">
					<input type="text" name="product_name" value="" style="width:15%">
				</div>
			</div>
			<div class="content__wrap">
			...
			<div class="card__body--hide detail_hidden" style="display: none;">
				<div class="content__wrap">
					...
					<div class="content__row">
						<input type="text" name="material" value="" style="width:15%">
					</div>
				</div>
				<div class="content__wrap">
					<div class="content__title">그래픽</div>
					<div class="content__row">
						<input type="text" name="graphic" value="" style="width:15%">
					</div>
				</div>
				<div class="content__wrap">
					<div class="content__title">핏</div>
					<div class="content__row">
						<input type="text" name="fit" value="" style="width:15%">
					</div>
				</div>
				<div class="content__wrap">
					<div class="content__title">사이즈</div>
					<div class="content__row">
						<input type="text" name="product_size" value="" style="width:15%">
					</div>
				</div>
				<div class="content__wrap">
					<div class="content__title">색상</div>
					<div class="content__row">
						<input type="text" name="color" value="" style="width:15%">
					</div>
				</div>
				<div class="content__wrap">
					<div class="content__title">색상코드</div>
					<div class="content__row">
						<input type="text" name="color_rgb" value="" style="width:15%">
					</div>
				</div>
				<div class="content__wrap">
					<div class="content__title">네비게이션</div>
					<div class="content__row">
						<input type="text" name="navigation" value="" style="width:15%">
					</div>
				</div>
				<div class="content__wrap">
			...
			<div class="table table__wrap">
				<div class="table__filter">
					<div class="filrer__wrap">
						<div style="width: 140px;" class="filter__btn" action_type="toggle_update_flg" onclick="ordersheetActionCheck(this)">진행상태 변경(md전용:임시)</div>
	...
	get_contents($("#frm-filter"),{
		...
		html : function(d) {
			...
			d.forEach(function(row) {
				if(row.ordersheet_img_location != null){
					var img_location = row.ordersheet_img_location.replace('/var/www/admin/www','')
				}
				var img_location = "background-image:url('" + img_location + "');";
                var background_url 		= img_location;
				var strDiv 				= "";
				var update_flg_str 		= "";
				var btn_class 			= "";
				var btn_onclick_func 	= "";
				console.log(background_url);
				if(row.ordersheet_update_flg == 0){
					update_flg_str = "진행 중";
					btn_class = "white_btn";
				...
				sampleBtn = `<button type="button" class="white_btn" onclick="location.href='/product/sample/insert?ordersheet_idx=${row.ordersheet_idx}&product_code=${row.product_code}'">등록</button>`;

				switch(row.sample_flg || row.wholesale_flg){
					case false:
						wholesaleBtn = `<button type="button" class="gray_btn" ordersheet_idx="${row.ordersheet_idx}" disable>등록</button>`;
						break;
					case true:
						wholesaleBtn = `<button type="button" class="white_btn" onclick="location.href='/product/wholesale/insert?ordersheet_idx=${row.ordersheet_idx}&product_code=${row.product_code}'">등록</button>`;
						break;
				...
				switch(row.wholesale_flg || row.factory_flg){
					case false:
						...
						break;
					case true:
						factoryBtn = `<button type="button" class="white_btn" onclick="location.href='/product/factory/insert?ordersheet_idx=${row.ordersheet_idx}&product_code=${row.product_code}'">등록</button>`;
						break;
                ...
                strDiv = `
                        <tr> 
				            ...
				            <td>${row.num}</td>
				            <td>
                                <button type="button" class="${btn_class}" ordersheet_idx="${row.ordersheet_idx}" authority="md" page_url="" onclick="${btn_onclick_func}">md 편집</button>
								<button type="button" class="${btn_class}" ordersheet_idx="${row.ordersheet_idx}" authority="dsn" onclick="${btn_onclick_func}">dsn 편집</button>
								<button type="button" class="${btn_class}" ordersheet_idx="${row.ordersheet_idx}" authority="td" onclick="${btn_onclick_func}">td 편집</button>
                            </td>
...
function ordersheetAction(len){
	...
	$.ajax({
		...
		data: formData,
		dataType: "json",
		url: config.api + "pm/ordersheet/"+api_str,
		error: function() {
...
function openOrdersheetUpdate(obj) {
	...
	var url = "/product/ordersheet/update/" + authority + "?";
