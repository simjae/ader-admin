...
function rowsChange(obj) {
	var rows = $(obj).val();
	$(obj).parents('.order_tab').find('form').eq(0).find('.rows').val(rows);
	$(obj).parents('.order_tab').find('form').eq(0).find('.page').val(1);
...
function setOrderBody(column_arr,d,order_status) {
	...
	d.forEach(function(row) {
		...
		if (detail_link.length > 0 && row.price_mileage > 0) {
		...
		strCheckboxDiv += '            </label>';
		strCheckboxDiv += '        </div>';
		strCheckboxDiv += '        <input class="order_refund_cnt" type="hidden" value="' + row.order_refund_cnt + '">';
		strCheckboxDiv += '    </td>';
		strCheckboxDiv += '</tr>';
		...
		if (order_product_info.length > 0 && order_product_info != null) {
			...
			order_product_info.forEach(function(product_row) {
				...
				strDiv += '                    <p style="margin-bottom:5px;">PRODUCT_NAME_3</p>';
				strDiv += '                    <div class="product__img__wrap">';
				strDiv += '                        <div class="product__img" style="' + background_url + '"></div>';
				strDiv += '                        <div>';
				strDiv += '                            <p>PRODUCT_NAME_3</p><br/>';
				strDiv += '                            <p style="color:#EF5012">2023-03-06 11:37:56</p>';
				strDiv += '                        </div>';
				strDiv += '                    </div>';
				strDiv += '                </div>';
		...
		if (row.order_exchange_cnt > 0)  {
			if (row.mapping_cnt == 0) {
				strDiv += '    <div class="btn" style="width:90px;text-align:center;" onClick="openOrderExchangeModal(\'' + row.order_code + '\');">교환정보추가</div>';
			} else {
				strDiv += '    <div class="btn" style="width:90px;text-align:center;margin-top:5px;" order_code="' + row.order_code + '"onClick="getIndpUrl(this);">바로구매URL</div>';
			}
		}
		strDiv += '    </td>';
...
function getOrderInfoList(order_status) {
	...
	strDiv += '    <TD class="default_td marker_td" colspan="' + (14 + column_arr.length - 1) + '" style="text-align:left;">';
	strDiv += '        조회 결과가 없습니다';
	strDiv += '    </TD>';
	strDiv += '</TBODY>';
...
function putOrderInfo(order_status) {
	let result_table = $('#result_checkbox_table_' + order_status);
	
	let checkbox = result_table.find('.checkbox');
	let order_refund_cnt = result_table.find('.order_refund_cnt');
	let cnt = checkbox.length;
	
	let order_idx = [];
	for (let i=0; i<cnt; i++) {
		if (checkbox.eq(i).prop('checked') == true && order_refund_cnt.eq(i).val() > 0) {
			order_idx.push(checkbox.eq(i).val());
		}
	}
	
	if (order_idx.length > 0) {
		confirm(
			'선택한 주문정보중를 환불완료 처리하시겠습니까?',
			function() {
				$.ajax({
					type: "post",
					url: config.api + "order/pg/put",
					data: {
						'order_idx' : order_idx
					},
					dataType: "json",
					...
					success: function(d) {
						if (d.code == 200) {
							...
							alert('선택한 주문의 환불완료 처리가 완료되었습니다.');
						} else {
							alert(d.msg);
						}
					}
		...
		alert('환불완료 처리할 주문정보를 선택해주세요.');
		return false;
	}
}

function getIndpUrl(obj) {
	let order_code = $(obj).attr('order_code');
	
	$.ajax({
		type: "post",
		url: config.api + "order/pg/get",
		data: {
			'order_code' : order_code
		},
		dataType: "json",
		error: function() {
			alert('바로구매URL 조회처리중 오류가 발생했습니다.');
		},
		success: function(d) {
			if (d.code == 200) {
				$('#indp_url_' + order_code).remove();
				$(obj).parent().append('<input id="indp_url_' + order_code + '" type="text" style="width:100%;margin-top:5px;display:block;" value="' + d.data + '">');
			} else {
				alert(d.msg);
			}
		}
...
function openOrderExchangeModal(order_code) {
	modal('/put','order_code=' + order_code);
}
