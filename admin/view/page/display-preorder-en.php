<div class="content__card">
	<div class="card__header">
		<div class="flex justify-between">
			<h3>영문몰 프리오더 검색</h3>
			<div class="black-btn" onclick="moveRegistPage('EN')">영문몰 프리오더 등록</div>
		</div>
		<div class="drive--x"></div>
	</div>

	<div class="card__body">
		<form id="frm-preorder-filter_en" action="order/preorder/list/get">
			<?php
	...
	<div class="card__footer">
		<div class="footer__btn__wrap">
			...
			<div class="btn__wrap--lg">
				<div class="blue__color__btn" onClick="getPreorderListEn();"><span>검색</span></div>
				<div class="defult__color__btn" onClick="resetMenu('EN');"><span>초기화</span></div>
			</div>
		</div>
	</div>
</div>
	...
	<div class="card__header">
		<div class="flex justify-between" style="gap:20px;">
			<div class="flex items-center" style="gap: 20px;">
				<h3>한국몰 프리오더 검색 결과</h3>
			</div>
		</div>
		<div class="drive--x"></div>
	</div>
	<div class="card__body">
		<form id="frm-preorder-list_en">
			<div class="info__wrap " style="justify-content:space-between; align-items: center;">
				...
				<div class="content__row">
					<select style="width:163px;float:right;margin-right:10px;" onChange="orderChange(this);">
						...
						<option value="SALES_PRICE_EN|DESC">판매가(힌국몰) 역순</option>
						<option value="SALES_PRICE_EN|ASC">판매가(힌국몰) 순</option>
			...
			<div class="table table__wrap">
				...
				<div class="overflow-x-auto">
					<table>
						...
						<thead>
							<tr>
								<th>
									<div class="cb__color">
										<label>
											<input type="checkbox" class="select" name="preorder_idx[]" value="">
											<span></span>
										</label>
									</div>
								</th>
						...
						<tbody id="preorder_result_table_en"></tbody>
					</table>
				</div>
			</div>
		</form>
...
function getPreorderListEn(){
	$("#preorder_result_table_en").html('');
	var strDiv = '';
	strDiv += '<TD class="default_td" colspan="15">';
	strDiv += '    조회 결과가 없습니다';
	strDiv += '</TD>';
	
	$("#preorder_result_table_en").append(strDiv);
	
	var rows = $('#frm-preorder-filter_en').find('.rows').val();
	var page = $('#frm-preorder-filter_en').find('.page').val();
	
	get_contents($("#frm-preorder-filter_en"),{
		pageObj : $("#frm-preorder-list_en").find(".paging"),
		html : function(d) {
			if (d.length > 0) {
				$("#preorder_result_table_en").html('');
			}
			...
			d.forEach(function(row) {
				...
				strDiv = `
					<tr>
						<td rowspan="${rowspan_num}">
							<div class="cb__color">
								<label>
									<input type="checkbox" class="select" name="preorder_idx[]" value="${row.preorder_idx}" ">
									<span></span>
								</label>
							</div>
						</td>
						...
						<td rowspan="${rowspan_num}">
							...
							<div class="product__img__wrap">
								...
								<span>
									...
									<p>${parseInt(row.sales_price).toLocaleString('ko-EN')} ₩</p><br>
				...
				row.qty_info.forEach(function(qty_row, index){
					optionDiv = `
						...
						<td>0</td>
						<td>
							<div class="btn" onclick="viewPreorderEntry(${row.preorder_idx}, ${qty_row.option_idx},'EN')">조회</div>
						</td>
					`;
					if(index != 0){
				...
				strDiv += `
						<td rowspan="${rowspan_num}">
							<div class="btn" onclick="viewPreorderEntry(${row.preorder_idx}, 'null', 'EN')">전체조회</div>
						</td>
						<td rowspan="${rowspan_num}">${row.sales_price}원</td>
						<td rowspan="${rowspan_num}">${row.entry_start_date}<br>- ${row.entry_end_date}</td>
						<td rowspan="${rowspan_num}">
							<div class="btn" onclick="moveUpdatePage('EN', ${row.preorder_idx})">편집</div>
						</td>
						<td rowspan="${rowspan_num}">
