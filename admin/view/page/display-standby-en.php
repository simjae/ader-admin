<div class="content__card">
	<div class="card__header">
		<div class="flex justify-between">
			<h3>영문몰 스탠바이 검색</h3>
			<div class="black-btn" onclick="moveRegistPage('EN')">영문몰 스탠바이 등록</div>
		</div>
		<div class="drive--x"></div>
	</div>

	<div class="card__body">
		<form id="frm-standby-filter_en" action="order/standby/list/get">
			<?php
	...
	<div class="card__footer">
		<div class="footer__btn__wrap">
			...
			<div class="btn__wrap--lg">
				<div class="blue__color__btn" onClick="getStandbyListEn();"><span>검색</span></div>
	...
	<div class="card__header">
		<div class="flex justify-between" style="gap:20px;">
			<div class="flex items-center" style="gap: 20px;">
				<h3>영문몰 스탠바이 검색 결과</h3>
			</div>
		</div>
		<div class="drive--x"></div>
	</div>
	<div class="card__body">
		<form id="frm-standby-list_en">
			...
			<div class="table table__wrap">
				...
				<div class="overflow-x-auto">
					<table>
						...
						<thead>
							<tr>
								<th>
									<div class="cb__color">
										<label>
											<input type="checkbox" class="select" name="draw_idx[]" value="" onclick="selectAllClick(this,'en')">
											<span></span>
										</label>
									</div>
								</th>
						...
						<tbody id="standby_result_table_en">
						</tbody>
					</table>
				</div>
			</div>
...
function getStandbyListEn(){
	$("#standby_result_table_en").html('');
	var strDiv = '';
	strDiv += '<TD class="default_td" colspan="15">';
	strDiv += '    조회 결과가 없습니다';
	strDiv += '</TD>';
	
	$("#standby_result_table_en").append(strDiv);
	
	var rows = $('#frm-standby-filter_en').find('.rows').val();
	var page = $('#frm-standby-filter_en').find('.page').val();
	
	get_contents($("#frm-standby-filter_en"),{
		pageObj : $("#frm-standby-list_en").find(".paging"),
		html : function(d) {
			if (d.length > 0) {
				$("#standby_result_table_en").html('');
			}
			...
			d.forEach(function(row) {
				...
				row.qty_info.forEach(function(qty_row, index){
					optionDiv = `
						...
						<td>0</td>
						<td>
							<div class="btn" onclick="viewStandbyEntry(${row.standby_idx}, ${qty_row.option_idx},'EN')">조회</div>
						</td>
					`;
					if(index != 0){
				...
				strDiv += `
						<td rowspan="${rowspan_num}">
							<div class="btn" onclick="viewStandbyEntry(${row.standby_idx}, 'null', 'EN')">전체조회</div>
						</td>
						...
						<td rowspan="${rowspan_num}">
							<div class="btn" onclick="moveUpdatePage('EN', ${row.standby_idx})">편집</div>
						</td>
						<td rowspan="${rowspan_num}">
