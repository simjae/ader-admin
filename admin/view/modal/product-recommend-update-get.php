...
<div class="content__card modal__view" style="max-width:1000px;margin: 0;">
	...
	<div class="card__body" style="width:1000px;">
		<form id="frm-filter_REL" action="product/relevant/list/get">
			...
			<input type="hidden" name="recommend_idx" value="<?=$recommend_idx?>">
	...
	get_contents(frm,{
		...
		html : function(d) {
			...
			d.forEach(function(row){
				...
				strDiv += '    <td>';
				strDiv += '        <div class="btn" onClick="addTmpRecommendProduct(' + row.product_idx + ')">선택</div>'
				strDiv += '    </td>';
...
function addTmpRecommendProduct(product_idx) {
	$.ajax({
		url: config.api + "product/recommend/modal/get",
		type: "post",
		data: {
		...
		success: function(d) {
			if(d.code == 200) {
				...
				if (data != null) {
					...
					strDiv += '                <div class="btn display_num_btn" onclick="displayNumCheck(' + data.product_idx + ',\'up\')">';
					strDiv += '                    <i class="xi-angle-left"></i>';
					strDiv += '                    <span class="tooltip top">위로</span>';
					strDiv += '                </div>';
					strDiv += '                <div class="btn display_num_btn" onclick="displayNumCheck(' + data.product_idx + ',\'down\')">';
