	...
	get_contents($("#frm-product_add"),{
		...
		html : function(d) {
			...
			d.forEach(function(row) {
				...
				strDiv += '<TR>';
				strDiv += '    <td>';
				strDiv += '        <button class="table__setting__btn" product_idx="' + row.product_idx + '" onclick="addStanbyProduct(this)">선택</button>';
				strDiv += '    </td>';
...
function addStanbyProduct(obj) {
	confirm('스텐바이 상품으로 지정하시겠습니까?',function(){
        ...
        $.ajax({
            ...
            success: function(d) {
                ...
                if (code == 200) {
					...
					alert('상품이 선택되었습니다. 스텐바이 등록을 진행해주세요', function (){
						modal_close();
					})
                } else {
                    alert('상품 읽기에 실패했습니다.');
...
function addSetProduct() {
    confirm('스텐바이 상품으로 결정하시겠습니까?',function(){
