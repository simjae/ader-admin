...
?>
		<h3>
		[<?=$box_data['BOX_NAME']?>]박스를 사용중인 상품 목록
...
?>
	...
	<div class="card__body" style="margin-top:50px">
		<form id="frm-box_product" action="pcs/ordersheet/modal/get">
			<input type="hidden" name="box_idx" value="<?=$box_idx?>">
			...
			<div class="card__footer">
				<div class="footer__btn__wrap">
					...
					<div class="btn__wrap--lg">
						...
						<div class="defult__color__btn" onClick="init_fileter('frm-box_product','getModalProductList()');"><span>초기화</span></div>
					</div>
				</div>
			</div>
		</form>
...
function changeSearchType(obj) {
	...
	var length = $('#frm-box_product').find('.search_type_select').length;
	
	if (length > 1) {
		var search_type_arr = [];
		for (var i=0; i<length; i++) {
			search_type_arr[i] = $('#frm-box_product').find('.search_type_select').eq(i).val();
		}
...
function changeOrderProduct(obj) {
	...
	$('#frm-box_product').find('.sort_value').val(order_value[0]);
	$('#frm-box_product').find('.sort_type').val(order_value[1]);
...
function changeRowsProduct(obj) {
	...
	$('#frm-box_product').find('.rows').val(rows);
	$('#frm-box_product').find('.page').val(1);
...
function getModalProductList() {
	...
	let rows = $("#frm-box_product").find('.rows').val();
	let page = $("#frm-box_product").find('.page').val();
	
	get_contents($("#frm-box_product"),{
