		...
		html : function(d) {
			if(d != null){
				...
				d.forEach(function(row) {
					...
					strDiv += '<TR>';
					strDiv += '    <td>';
					strDiv += '        <button class="table__setting__btn" product_idx="' + row.product_idx + '" onclick="addPreorderProduct(this)">선택</button>';
					strDiv += '    </td>';
...
function addPreorderProduct(obj) {
	confirm('프리오더 상품으로 지정하시겠습니까?',function(){
        ...
        $.ajax({
            ...
            success: function(d) {
                ...
                if (code == 200) {
					...
					alert('상품이 선택되었습니다. 프리오더 등록을 진행해주세요', function (){
						modal_close();
					})
                } else {
                    alert('상품 읽기에 실패했습니다.');
...
function addSetProduct() {
    confirm('프리오더 상품으로 결정하시겠습니까?',function(){
