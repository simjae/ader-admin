...
$(document).ready(function(){
    getEditorialProductCategory();
})

function getEditorialProductCategory() {
	$('.js--tree').remove();
	...
	$('.js--tree').jstree({
		...
		getEditorialProduct(md_category_node,md_category_depth,page_idx);
	});
}

function getEditorialProduct(md_category_node,md_category_depth,page_idx) {
	let result_table = $('.result_table_product');
	
	$.ajax({
		type: "post",
		url: config.api + "display/posting/editorial/product/list/get",
		data: {
		...
		dataType: "json",
		error: function() {
			alert("에디토리얼 상품 조회처리중 오류가 발생했습니다.");
		},
		success: function(d) {
			if(d.code == 200) {
				let data = d.data;
				...
				if (data != null) {
					...
					data.forEach(function(row) {
						...
						if (action_type == "ADD") {
							strDiv += '    <div class="btn" style="width:80px;text-align:center;" action_type="ADD" product_idx="' + row.product_idx + '" onClick="putEditorialProduct(this);">선택</div>';
						} else if (action_type == "DEL") {
							strDiv += '    <div class="btn" style="width:80px;text-align:center;background-color:#000000;color:white;" action_type="DEL" product_idx="' + row.product_idx + '" onClick="putEditorialProduct(this);">선택완료</div>';
						}
...
function putEditorialProduct(obj) {
	...
	$.ajax({
		...
		dataType: "json",
		error: function() {
			alert("에디토리얼 상품 선택/해제 처리중 오류가 발생했습니다.");
		},
		success: function(d) {
			if(d.code == 200) {
				let strDiv = "";
				if (action_type == "ADD") {
					strDiv += '    <div class="btn" style="width:80px;text-align:center;background-color:#000000;color:white;" action_type="DEL" product_idx="' + product_idx + '" onClick="putEditorialProduct(this);">선택완료</div>';	
				} else if (action_type == "DEL") {
					strDiv += '    <div class="btn" style="width:80px;text-align:center;" action_type="ADD" product_idx="' + product_idx + '" onClick="putEditorialProduct(this);">선택</div>';	
				}
				...
				alert("에디토리얼 상품 선택/해제 처리에 실패했습니다. 관련상품을 확인해주세요.");
			}
		}
	});
}
