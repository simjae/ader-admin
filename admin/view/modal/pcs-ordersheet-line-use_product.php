...
?>
		<h3>
		[<?=$line_data['LINE_NAME']?>]라인을 사용중인 상품 목록
...
?>
	...
	<div class="card__body" style="margin-top:50px">
		<form id="frm-line_product" action="pcs/ordersheet/modal/get">
			<input type="hidden" name="line_idx" value="<?=$line_idx?>">
			...
			<div class="card__footer">
				<div class="footer__btn__wrap">
					...
					<div class="btn__wrap--lg">
						...
						<div class="defult__color__btn" onClick="init_fileter('frm-line_product','getModalProductList()');"><span>초기화</span></div>
					</div>
				</div>
			</div>
		</form>
...
function changeSearchType(obj) {
	...
	var length = $('#frm-line_product').find('.search_type_select').length;
	
	if (length > 1) {
		var search_type_arr = [];
		for (var i=0; i<length; i++) {
			search_type_arr[i] = $('#frm-line_product').find('.search_type_select').eq(i).val();
		}
...
function changeOrderProduct(obj) {
	...
	$('#frm-line_product').find('.sort_value').val(order_value[0]);
	$('#frm-line_product').find('.sort_type').val(order_value[1]);
...
function changeRowsProduct(obj) {
	...
	$('#frm-line_product').find('.rows').val(rows);
	$('#frm-line_product').find('.page').val(1);
...
function getModalProductList() {
	...
	let rows = $("#frm-line_product").find('.rows').val();
	let page = $("#frm-line_product").find('.page').val();
	
	get_contents($("#frm-line_product"),{
