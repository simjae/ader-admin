...
<div class="content__card modal__view" style="margin: 0;">
	...
	<div class="card__body">
		...
		<div class="content__card">
			<form id="frm-voucher_product">
				<input type="hidden" id="voucher_idx" value="<?=$voucher_idx?>">
				<div class="card__header">
					<h3>상품 목록 검색 결과</h3>
				...
				<div class="card__body">
					...
					<div class="table table__wrap">
						<div class="table__filter">
							<input id="voucher_product_list" type="hidden" value="">
							<div class="tmp_voucher_product_wrap">
	...
	<div class="card__footer">
		<div class="footer__btn__wrap">
			...
			<div class="btn__wrap--lg">
				<div  class="blue__color__btn" onClick="addvoucherProduct();"><span>바우처 상품 추가</span></div>
				<div class="defult__color__btn" onClick="init_fileter('frm-filter','getModalProductList');"><span>초기화</span></div>
			</div>
		</div>
	</div>
</div>
	...
	if ($(obj).prop('checked') == true) {
		...
		if (checkbox_type == "modal_product") {
			...
			for (let i=0; i<cnt; i++) {
				...
				let result = checkvoucherProduct(product_idx);
				if (result == false) {
					break;
				}
			}
		...
		if (checkbox_type == "modal_product") {
			...
			for (let i=0; i<cnt; i++) {
				...
				removevoucherProduct(product_idx);
			}
		}
	}
}
...
function getModalProductList() {
	...
	let voucher_product_list = $('#voucher_product_list');
	let tmp_product_idx = [];
	if (voucher_product_list.val().length > 0) {
		tmp_product_idx = voucher_product_list.val().split(',');
	}
	...
	get_contents($("#frm-product_add"),{
		...
		html : function(d) {
			...
			d.forEach(function(row) {
				...
				strDiv += '            <label>';
				strDiv += '                <input id="modal_product_checkbox_' + row.product_idx + '" class="modal_product_checkbox" type="checkbox" name="product_idx[]" value="' + row.product_idx + '" onChange="clickvoucherProduct(this);" ' + checked_str + '>';
				strDiv += '                <span></span>';
				strDiv += '            </label>';
				strDiv += '        </div>';
				strDiv += '    </td>';
...
function clickvoucherProduct(obj) {
	let checked = $(obj).prop('checked');
	let product_idx = $(obj).val();
	
	if (checked == true) {
		checkvoucherProduct(product_idx);
	} else {
		removevoucherProduct(product_idx);
	}
}

function checkvoucherProduct(product_idx) {
	let voucher_product_list = $('#voucher_product_list');
	let tmp_idx_arr = [];
	
	if (voucher_product_list.val().length > 0) {
		tmp_idx_arr = voucher_product_list.val().split(',');
	}
	...
	if (result < 0) {
		...
		voucher_product_list.val(tmp_idx_arr);
		
		addTmpvoucherProduct(product_idx);
	} else {
		alert('중복된 상품을 바우처 상품으로 등록할 수 없습니다.');
		return false;
	}
}

function removevoucherProduct(product_idx) {
	let product_idx_arr = [];
	$('.voucher_product_' + product_idx).remove();
	$('#modal_product_checkbox_' + product_idx).prop('checked',false);
	
	let cnt = $('.tmp_voucher_product').length;
	for (let i=0; i<cnt; i++) {
		let tmp_product_idx = $('.tmp_voucher_product').eq(i).attr('product_idx');
		product_idx_arr.push(tmp_product_idx);
	}
	
	$('#voucher_product_list').val(product_idx_arr);
}

function addTmpvoucherProduct(product_idx) {
	$.ajax({
		...
		success: function(d) {
			...
			if (code == 200) {
				...
				let strDiv = '<div class="tmp_voucher_product voucher_product_' + data[0].product_idx + '" product_idx="' + data[0].product_idx + '" onClick="removevoucherProduct(' + data[0].product_idx + ')">' + data[0].product_code + ' | ' + data[0].product_name + '</div>';
				$('.tmp_voucher_product_wrap').append(strDiv);
			} else {
				alert('바우처 상품 추가 처리에 실패했습니다. 추가하려는 바우처 상품을 확인해주세요.');
				return false;
			}
		}
	});
...
function addvoucherProduct() {
    confirm('선택한 상품을 바우처 상품으로 등록하시겠습니까?',function(){
        //let voucher_product_list = $('#voucher_product_list').val();
        var list_cnt = $('.tmp_voucher_product_wrap').children().length;
        //$('#relevant_product_list_area').css('visibility','visible');
        $('#voucher_product_table').html('');

		if(list_cnt > 0){
			for(var i = 0; i < list_cnt; i++){
				var sel_prod = $('.tmp_voucher_product_wrap').children().eq(i);
				...
				$.ajax({
					...
					success: function(d) {
						...
						if (code == 200) {
							...
							//$('#voucher_list_area').css('visibility','visible');
							modal_close();
