...
function eventAddAction(){
	...
	var event_always = $('#frm-add').find("input[name='event_always']:checked").val();
	...
	confirm("이벤트를 등록하시겠습니까?",function() {
		$.ajax({
			type: "post",
			data: formData,
			dataType: "json",
			url: config.api + "event/add",
			error: function() {
			...
			success: function(d) {
				if(d.code == 200) {
					...
					alert("이벤트 등록에 성공했습니다.",function(){
						getEventInfoList(formData.country);
                        modal_close();
                    });
				}
			}
