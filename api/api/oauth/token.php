...
$client_key = (isset($client_key))?trim($client_key):NULL;
$where = 'CLIENT_ID=? AND AUTH_KEY=?';
$where_values = array($client_id,$client_key);
...
elseif($client_key == NULL) {
	$code = 761;
	$result = false;
}
elseif(strlen($client_key) != 20) {
	$code = 762;
	$result = false;
}
...
elseif(!in_array($grant_type,GRANT_TYPE)) {
	$code = 764;
	$result = false;
}
elseif($db->count($_TABLE['OAUTH'],$where,$where_values) == 0) {
	...
	elseif($data['IP'] != '0.0.0.0' && $data['IP'] != $_SERVER['REMOTE_ADDR']) {
		...
		*/
		$access_token = base64_encode(openssl_encrypt(substr(getmicrotime().$client_id.$client_key,0,30), 'aes-256-cbc', AES_PASSWORD, OPENSSL_RAW_DATA, AES_IV_128)); // 암호화

		// 재발행 토큰 유효기간
		$refresh_timestamp = time()+(REFRESH_TOKEN_EXPIRE_TIME*60)+(9*60*60);
		$refresh_datetime = date('Y-m-d H:i:s',$refresh_timestamp);
		$refresh_token = base64_encode(openssl_encrypt(substr($client_id.getmicrotime().$client_key,0,30), 'aes-256-cbc', AES_PASSWORD, OPENSSL_RAW_DATA, AES_IV_128)); // 암호화
